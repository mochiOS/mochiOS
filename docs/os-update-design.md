...
- Systemイメージは全パーティションをSHA-256し、version、build、architecture、image size、`SHA-256(raw Ed25519 public key)`のkey IDとともに固定長manifestへEd25519署名する。manifestはESPのslot別ディレクトリへ置く。mBootは選択slotと同じ物理ディスク上のSystem A/BをGPT種別と順序から一意に解決し、全ブロックを読み切ってdigestと固定公開鍵による署名を確認した後にだけkernelをロードする。manifest欠落、不明鍵、不正形式、サイズ・architecture・digest・署名不一致、System領域の個数異常はすべてfail closedとする。`DEVELOPMENT_SYSTEM_SIGNATURES=y`はKVM専用の公開テスト鍵を追加する設定であり、本番既定値は無効。本番イメージ生成にはリポジトリ外の`MOCHIOS_SYSTEM_SIGNING_KEY`が必要である。mBoot自身とESP上のkernel/initfsを信頼するにはUEFI Secure Bootなど上位のboot chainも別途必要であり、System署名だけでその代替にはならない。
