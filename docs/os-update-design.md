...
- `mmake ab-layout-image`はESP、System A、System B、Data、専用boot-stateの5 GPT領域を持つイメージを`out/mmake/image/ab-layout.img`へ作る。ESPには`/slots/A`と`/slots/B`のkernel、kernel.meta、initfsを別々に収める。System A/Bには`/system`だけを格納し、標準コマンド、標準アプリ、サービス、組み込みパッケージ、システムフォントなどのOS所有物を配置する。Dataには`/bin`、`/applications`、`/libraries`、`/home`、`/var`、`/tmp`を格納する。`/libraries`は追加フォントなどを置けるData上の共有ライブラリ・リソース領域であり、システムフォントは`/system/libraries/fonts`に置く。

- ext2 CEXTは選択されたSystemスロットを読み取り専用として扱う。`/system`への変更は拒否する。互換名前空間である`/bin`と`/applications`はSystemとDataを統合表示するが、System側の同名項目をDataから変更・置換できない。`/libraries`、`/home`、`/var`、`/tmp`はDataだけに属する。アカウントDB、設定、ログ、証明書DB、パッケージ状態、診断キューなどの可変状態をSystemへ保存してはならない。

- slot manifest v2はSystemパーティション全体、kernel、kernel.meta、initfsの各SHA-256と、version、build、architecture、System image size、`SHA-256(raw Ed25519 public key)`のkey IDを固定長headerへ格納し、全体をEd25519署名する。mBootは選択slotと同じ物理ディスク上のSystem A/Bを一意に解決し、4成果物すべてを読み切ってdigestと固定公開鍵による署名を確認した後にだけkernelをロードする。manifest欠落、不明鍵、不正形式、サイズ・architecture・digest・署名不一致、System領域の個数異常はすべてfail closedとする。`mmake boot-assets-signature-smoke-test-kvm`はkernelとinitfsを各1バイト変更したdiskをKVM起動し、どちらもkernel開始前に拒否されることを確認する。`DEVELOPMENT_SYSTEM_SIGNATURES=y`はKVM専用の公開テスト鍵を追加する設定であり、本番既定値は無効。本番イメージ生成にはリポジトリ外の`MOCHIOS_SYSTEM_SIGNING_KEY`が必要である。
- mBootはPE/COFF Authenticode署名済みの`BOOTX64.EFI`として生成する。本番ビルドはリポジトリ外の`MOCHIOS_UEFI_SIGNING_KEY`と`MOCHIOS_UEFI_SIGNING_CERT`を必須とし、`sbsign`後に`sbverify`する。KVMだけは`DEVELOPMENT_UEFI_SIGNING=y`でOVMFの公開snakeoil fixtureを使用できる。`REQUIRE_UEFI_SECURE_BOOT=y`のmBootはUEFIの`SecureBoot=1`かつ`SetupMode=0`を確認できない場合にfail closedとする。`mmake secure-boot-smoke-test-kvm`はSecure Boot有効OVMFで署名済みmBootの起動を確認し、ESPを未署名mBootへ差し替えた場合はmBootのentry pointより前にUEFIが拒否することを確認する。開発鍵を本番のdbへ登録してはならない。
- anti-rollbackの正本を書換え可能なdisk上のboot-stateには置かない。mBootは固定vendor GUIDのUEFI不揮発変数`MochiOSRollbackFloor`を`NON_VOLATILE | BOOTSERVICE_ACCESS`だけで保持し、OS runtimeからの読書きを許可しない。署名検証済みbuildがfloor未満なら拒否する。pending trialではfloorを進めず、boot-state上でstableになったslotだけが署名検証後、kernelロード前にfloorを単調増加させ、書込み後の読戻しまで確認する。変数の欠落時はmBootへ埋め込んだ`MOCHIOS_MINIMUM_BUILD`を下限にし、変数の不正形式・読書き失敗はfail closedとする。`mmake anti-rollback-smoke-test-kvm`は署名済みbuild 2を起動して同じOVMF変数へfloorを永続化し、その状態で正規署名済みbuild 1がkernel開始前に拒否されることを確認する。UEFI鍵・変数をplatform ownerが初期化する物理的なrecovery操作は別の信頼境界である。
