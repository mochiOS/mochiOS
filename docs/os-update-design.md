...
- `mmake ext2-write-test`はKVMでprepare、再起動後の永続性、容量不足、各段階の`e2fsck`まで完了した。テスト専用fixtureは`mmake ext2-write-fixture-test`で起動前に検査する。ext2 CEXTはGPTで選択したsystem領域の開始・終端を保持し、filesystem容量と各I/Oのパーティション境界を検査する。境界外I/Oのunit testは`mmake ext2-cext-test`で実行する。境界ガード追加後のKVM再確認と電源断中のext2書込み整合性は未実施である。ext2はjournalを持たないため、今回の成功を電源断耐性の証明とは扱わない。
