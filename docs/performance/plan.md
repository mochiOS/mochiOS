...
2026年9月1日のreleaseビルドは18,710,928 bytesです。このうち17,738,760 bytesを固定メールボックスが占めています。`.text`は539,574 bytesなので、最初に直すべきなのは命令数ではなく、未使用時にも実体を持つ固定領域です。詳しい内訳は[mnuカーネルのサイズ](mnu-kernel-size.md)に保存しています。
...
各メールボックスが最大メッセージ本文を抱える構造をやめます。本文はシステム全体で上限を持つ動的プールへ置き、メールボックスには小さな記述子だけを残します。枯渇時は明示的なエラーを返し、送信済みデータの所有権と回収経路を一つにします。

ここではIPCのCapability検査を維持します。共有ページを使う大きなIPCとは分け、小さいメッセージのために毎回heap allocationしない設計にします。
