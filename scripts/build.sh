...
need_dir "${ROOT_DIR}/libraries/newlib"
...
perl \
    "${SCRIPT_DIR}/build-signature-db.pl" \
    "${SIGNATURE_DB_ARGS[@]}"
...
echo "[step] stage filesystem images"
install -m 0644 \
    "${INITFS_IMG}" \
...
install -m 0644 \
    "${BOOT_BIN}" \
    "${ARTIFACT_DIR}/BOOTX64.EFI"

install -m 0755 \
    "${SERVICE_BIN}" \
    "${ARTIFACT_DIR}/core.service"

install -m 0644 \
    "${SIGNATURE_DB_STAGE}" \
    "${ARTIFACT_DIR}/signature.db"
